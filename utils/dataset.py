...
def dataloader(img_path, text_path):
    train_dataset = CustomImageDataset(
        text_file=text_path + "train_data.csv", img_dir=img_path
    )
    train_dataloader = DataLoader(train_dataset, batch_size=2, shuffle=True)
    validation_dataset = CustomImageDataset(
        text_file=text_path + "validation_data.csv", img_dir=img_path
    )
    validation_dataloader = DataLoader(train_dataset, batch_size=2)
    test_dataset = CustomImageDataset(
        text_file=text_path + "test_data.csv", img_dir=img_path
    )
